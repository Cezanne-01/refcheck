...
## 에이전트 모드 (use_agents=True)

- [ ] Streamlit UI에서 "🤖 에이전트 모드" 체크박스 체크 → 검증 실행
- [ ] 동일 초안을 기본 모드 vs 에이전트 모드로 각각 실행 → findings 수 비교
- [ ] 에이전트 모드에서 `abstract_insufficient` 카테고리가 `content_mismatch` 대신 많이 나오는지 확인
- [ ] 에이전트가 max_turns=6 안에 항상 submit_final 호출하는지 (타임아웃 없는지)
- [ ] 45개 이상 참고문헌 초안에서 비용이 $30 이하인지
